...
do
	...
	ip_address=`ifconfig | grep -A 3 mlan0 | awk '/inet addr/ {print $2}' | cut -f2 -d ":"`
...
while true
do
    ip_address_4G=`ifconfig | grep -A 3 ppp0 | awk '/inet addr/ {print $2}' | cut -f2 -d ":"`
    ip_address_5G=`ifconfig | grep -A 3 wwan0 | awk '/inet addr/ {print $2}' | cut -f2 -d ":"`
